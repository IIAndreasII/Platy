...
void ParticleEmitter::ResetParticle(const size_t& i)
{
	...
	case EParticleEmitterType::CLOUD:
		myParticles[i].velocity = Game::Core::Util::DegToVec2(Core::Util::RandFloat(0, 360));
		myParticleVertices[i * 4].position = GetShowerPosition();
		myParticleVertices[i * 4].position += sf::Vector2f(0, Core::Util::RandFloat(-20, 20));
		break;
	...
	case EParticleEmitterType::FOUNTAIN:
		myParticles[i].velocity = Game::Core::Util::DegToVec2(Core::Util::RandFloat(myEmissionAngle - mySpreadAngle, myEmissionAngle + mySpreadAngle));
		myParticleVertices[i * 4].position = myPosition;
		break;

	case EParticleEmitterType::SHOWER:
		myParticles[i].velocity = Game::Core::Util::DegToVec2(static_cast<int>(myOrientation) - myEmissionAngle);
		myParticleVertices[i * 4].position = GetShowerPosition();
		break;
	...
	float randWidth = Core::Util::RandFloat(2, myParticleSize);
	float randHeight = Core::Util::RandFloat(2, myParticleSize);
...
const sf::Vector2f ParticleEmitter::GetShowerPosition() const
{
	...
	{
	default:
		return sf::Vector2f(myPosition.x + Core::Util::RandFloat(0, myLength), myPosition.y);
	case EOrientation::UP:
		return sf::Vector2f(Core::Util::RandFloat(myPosition.x, myPosition.x + myLength), myPosition.y);
	case EOrientation::RIGHT:
		return sf::Vector2f(myPosition.x, Core::Util::RandFloat(myPosition.y, myPosition.y + myLength));
